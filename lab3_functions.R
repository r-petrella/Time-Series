...
KF <-function(y, phi, sigma_e, sigma_eta){
  ...
  m10 = 0    # cond exp of y_1|0
  P10 = 1   # variance of alpha_1|0
  ...
  for(t in 1:(n-1)){
    v[t] = y[t] - mu_pred[t];    # innovation error
    F[t] = P[t] + sigma_e^2;  
    K[t] = (phi * P[t])/F[t]
...
estimator <- function(y,par){
  ...
  hat = nlminb(start = theta_0, objective = loglikelihood, y = y)   # it minimizes the llh
